...
def train_crbfi(x, y, chosen_gamma, alpha):
    return np.linalg.solve(produce_kernel(x, x, chosen_gamma)+alpha*np.diag(np.ones(x.shape[0])), y) # shape = (300, 8), one weight for each training point and for each coil

def grid_evaluation(high_dim_x, y_train, x_val, y_val, chosen_gamma=.0005, alpha=1e-10):
    W = train_crbfi(high_dim_x, y_train, chosen_gamma, alpha)
...
def diag_evaluation(high_dim_x, y_train, x_diag, y_diag, chosen_gamma=.0005, alpha=1e-10):
    ...
    diag_predictions = np.matmul(diag_kernel, train_crbfi(high_dim_x, y_train, chosen_gamma, alpha)) # shape = (300, 8)
...
def visualize_diag_pred(diag_pred, y_diag, stdv=None, title="Magnetic fields components estimation", show=True, namefile=None):
